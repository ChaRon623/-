...
public class Parse {
	...
	public DefaultMutableTreeNode Parsing() {
		...
		System.out.println("栈中剩余元素："+deduce_str+", 栈剩余大小："+deduce_str.size());
		//System.out.println(input_cacheRow.get(0)+"行存在错误");
		
		// ---------------
		if (deduce_str.size() > 0 && input_cache.size() == 0) {
			if(deduce_str.size()!=1) {
				//文法出错
				gra_output.append("ERROR！推导未能完全终结，请检查源程序完整性");
				return tree[0];
			}
